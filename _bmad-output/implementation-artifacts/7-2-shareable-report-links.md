...
- [ ] Task 1: Prisma migration — shared_report_tokens table (AC: 1)
  - [ ] 1.1 Add `SharedReportToken` model to schema.prisma
  - [ ] 1.2 Fields: `id` UUID, `created_by` FK→users, `report_type`, `entity_id`, `period_month`, `period_year`, `snapshot_data` JSONB, `expires_at`, `revoked_at` (nullable), `created_at`
  - [ ] 1.3 Run `pnpm prisma migrate dev`

- [ ] Task 2: Share service (AC: 1, 3, 4, 5, 6)
  - [ ] 2.1 Create `services/share.service.ts` (or add to `report.service.ts`)
  - [ ] 2.2 `createShareLink(reportType, entityId, period, user)` — call report service to get current data, store as `snapshot_data`, set `expires_at = now + 30 days`
  - [ ] 2.3 `getSharedReport(token)` — query token row, check not expired + not revoked, return `snapshot_data`
  - [ ] 2.4 `revokeShareLink(tokenId)` — set `revoked_at = now()`
  - [ ] 2.5 Error handling: `GoneError` for expired/revoked tokens

- [ ] Task 3: Share routes (AC: 2, 3, 6, 7)
  - [ ] 3.1 Add to `routes/reports.routes.ts`:
  - [ ] 3.2 `POST /share` — `rbacMiddleware(['finance', 'admin'])`, `asyncHandler`
  - [ ] 3.3 `GET /shared/:token` — NO auth middleware (public endpoint), `asyncHandler`
  - [ ] 3.4 `DELETE /share/:tokenId` — `rbacMiddleware(['admin'])`, `asyncHandler`

- [ ] Task 4: Public shared report page (AC: 8)
  - [ ] 4.1 Create `pages/reports/SharedReport.tsx`
  - [ ] 4.2 Public route `/reports/shared/:token` — no AuthGuard
  - [ ] 4.3 Fetch from `GET /api/v1/reports/shared/:token`
  - [ ] 4.4 Banner: "This is a shared snapshot · Generated [date] · Expires [date]"
  - [ ] 4.5 Read-only render — no navigation, no filters

- [ ] Task 5: Frontend share button (AC: 2)
  - [ ] 5.1 "Share Link" button on dashboard pages (Finance/Admin only)
  - [ ] 5.2 Click → POST to share endpoint → copy URL to clipboard
  - [ ] 5.3 antd `message.success('Share link copied to clipboard')`

- [ ] Task 6: Rate limiting on public endpoint (AC: 3)
  - [ ] 6.1 Add `express-rate-limit` to `GET /shared/:token` — 60 req/min per IP

- [ ] Task 7: Zod schemas (AC: 1, 2)
  - [ ] 7.1 Add `shareRequestSchema` to `shared/schemas/dashboard.schema.ts`
  - [ ] 7.2 Add `shareResponseSchema`

- [ ] Task 8: Tests (AC: 9)
  - [ ] 8.1 Create `services/share.service.test.ts` (or `report.service.test.ts`)
  - [ ] 8.2 Test: Token creation with correct 30-day expiry
  - [ ] 8.3 Test: Valid token retrieval — returns snapshot_data
  - [ ] 8.4 Test: Expired token — 410 LINK_EXPIRED
  - [ ] 8.5 Test: Revoked token — 410 LINK_REVOKED
  - [ ] 8.6 Test: Admin revoke — sets revoked_at
  - [ ] 8.7 Test: Non-Admin revoke — 403
